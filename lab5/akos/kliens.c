...
void clean(int server_descriptor, int client_descriptor, char* client_query_name, int pid)
{
    if (server_descriptor != -1)
    {
        ...
        {
            fprintf(stderr, "[%d] - Client - Nem sikerult megnyitni a szerver uzenetsorat!\n", pid);
        }
        else
        {
            fprintf(stderr, "[%d] - Client - Szerver uzenetsora bezarva!\n", pid);
        }
    }
    ...
    if (client_descriptor != -1)
    {
        ...
        {
            fprintf(stderr, "[%d] - Client - Nem sikerult bezarni a sajat uzetesort!\n", pid);
        }
        else
        {
            fprintf(stderr, "[%d] - Client - Sajat uzenetsor bezarva!\n", pid);
        }
    }
    ...
    if (strcmp(client_query_name, ""))
    {
        ...
        {
            fprintf(stderr, "[%d] - Client - Nem sikerult torolni a sajat uzenetsort!\n", pid);
        }
        else
        {
            fprintf(stderr, "[%d] - Client - Sajat uzenetsor torolve!\n", pid);
        }
    }
}
...
int main(int argc, char** argv)
{
    ...
    //Osszetett parancs az argumentumokbol
    unsigned int i;
    size_t len = 0;
    ...
    *(_all_args-1) = 0;
    ...
    strcpy(msg_client.command, all_args);
    msg_client.pid = pid;
    ...
    //Sajat file letrehozasa
    char fileName[50];
    sprintf(fileName,"k_%d_out.dat",pid);
