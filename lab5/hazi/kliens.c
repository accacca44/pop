...
int main (int argc, char **argv)
{
    ...
    sprintf (clientQName, "/keim2152-%d", getpid ());
    ...
    if ((qd_client = mq_open (clientQName, O_RDONLY | O_CREAT, QUEUE_PERMISSIONS, &attr)) == -1) {
    ...
    while (fgets (temp_buf, MAX_MSG_SIZE, stdin)) {
        ...
        printf ("ENTER ANOTHER COMMAND, OR EXIT ON <ENTER> ");
    }
